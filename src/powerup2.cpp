...
Powerup2::Powerup2(float x, float y, color_t color) {
    ...
    int vertex_count=0,n=20;
    for(int i=0;i<n;i++)
    {
        vertex_buffer_data[vertex_count++]=0.4*cos((2* M_PI*i)/n);
        vertex_buffer_data[vertex_count++]=0.4*sin((2* M_PI*i)/n);
        vertex_buffer_data[vertex_count++]=0.0f;
        vertex_buffer_data[vertex_count++]=0.4*cos((2* M_PI*((i+1)%n))/n);
        vertex_buffer_data[vertex_count++]=0.4*sin((2* M_PI*((i+1)%n))/n);
        vertex_buffer_data[vertex_count++]=0.0f;
        vertex_buffer_data[vertex_count++]=0.0f;
        vertex_buffer_data[vertex_count++]=0.0f;
        vertex_buffer_data[vertex_count++]=0.0f;
